...
class PerplexitySearchAgent:
    """Agent for searching using Perplexity API via MCP."""
    
    def __init__(self, communication_bus: CommunicationBus, mcp_client: SimpleMCPClient):
        self.communication_bus = communication_bus
        self.mcp_client = mcp_client
        self.agent_id = f"perplexity_search_{uuid.uuid4().hex[:8]}"
        self.running = False
    
    async def start(self):
        """Start the agent."""
        self.running = True
        await self.communication_bus.subscribe("search.perplexity", self._handle_search_request)
        print(f"Perplexity Search Agent {self.agent_id} started")
    
    async def stop(self):
        """Stop the agent."""
        self.running = False
        await self.communication_bus.unsubscribe("search.perplexity", self._handle_search_request)
        print(f"Perplexity Search Agent {self.agent_id} stopped")
    ...
    async def _send_search_response(self, original_message: Dict[str, Any], 
        ...
        response = {
            ...
            "topic": "search.perplexity.response",
            "content": {
                "results": results,
